<script>
  ...
  function applyView(view) {
    ...
    units = view.units.map((unit) => ({ ...unit, audioState: audioByUnit[unit.id]?.audioState ?? unit.audioState }));
  }
  ...
  async function openProject() {
    try {
      ...
      if (view) { clearTimeout(playbackTimer); audioByUnit = {}; playingUnitId = null; applyView(view); }
    } catch (reason) { error = String(reason); }
  }
  ...
  async function reconcilePlayback(unitId) {
    if (playingUnitId !== unitId) return;
    try {
      const playing = await projectBridge.isUnitAudioPlaying(unitId);
      if (!playing) {
        playingUnitId = null;
        return;
      }
      playbackTimer = setTimeout(() => reconcilePlayback(unitId), 150);
    } catch (reason) {
      playingUnitId = null;
      error = String(reason);
    }
  ...
  async function playUnit(unit) {
    ...
    try {
      ...
      playbackTimer = setTimeout(() => reconcilePlayback(unit.id), 150);
    } catch (reason) { error = String(reason); }
  }
...
<main class="shell">
  <header>
    ...
    <div class="header-actions"><p class="status">{units.length} Absätze · Audio pro Render Unit</p><div class="project-actions" aria-label="Projektdatei"><button type="button" onclick={openProject}>Projekt öffnen</button><button type="button" class="primary" onclick={saveProject}>Projekt speichern</button></div></div>
  </header>
  ...
  <section class="workspace" aria-labelledby="units-heading">
    <div class="section-heading"><div><p class="label">RENDER UNITS</p><h2 id="units-heading">Absätze & Stimmen</h2></div><p>Stimme wird pro Absatz gewählt</p></div>
    <div class="units">
      {#each units as unit (unit.id)}
        <article class="unit">
          ...
          <label><span>Stimme</span><select value={unit.voiceId} onchange={(event) => setUnitVoice(unit.id, event.currentTarget.value)} disabled={unit.audioState === "rendering"}><option value="thorsten-high">Thorsten High</option><option value="thorsten-hessisch">Thorsten Hessisch</option></select></label>
          <div class="unit-actions">
            <button type="button" class="render-button" onclick={() => renderUnit(unit.id)} disabled={unit.audioState === "rendering"}>{unit.audioState === "rendering" ? "Rendert …" : "Rendern"}</button>
            {#if unit.audioState === "ready"}
              {#if playingUnitId === unit.id}<button type="button" onclick={() => stopUnit(unit.id)}>Stop</button>{:else}<button type="button" onclick={() => playUnit(unit)}>Anhören</button>{/if}
            {/if}
          </div>
        </article>
      {/each}
...
<style>
  ...
  button:hover { background: #f7f7f5; } button:disabled { cursor: default; opacity: .55; }
  button.primary { background: #292927; border-color: #292927; color: white; } button.primary:hover { background: #3a3a37; }
  .eyebrow, .label { margin: 0; font-size: 12px; letter-spacing: .12em; text-transform: uppercase; color: #777; }
  h1 { margin: 2px 0 0; font-size: 32px; font-weight: 650; } h2 { margin: 4px 0 0; font-size: 19px; font-weight: 600; }
  ...
  textarea { width: 100%; min-height: 220px; resize: vertical; border: 1px solid #d8d8d5; border-radius: 10px; padding: 16px; font: inherit; font-size: 16px; line-height: 1.55; background: #fff; } textarea:disabled { opacity: .6; }
  .units { display: grid; gap: 10px; }
  .unit { display: grid; grid-template-columns: 52px 1fr 190px 180px; gap: 16px; align-items: center; padding: 15px 0; border-top: 1px solid #ececea; } .unit:first-child { border-top: 0; }
  .unit-number { font-variant-numeric: tabular-nums; color: #777; font-size: 13px; }
  .unit-content p { margin: 0 0 6px; line-height: 1.4; } .audio-state { font-size: 12px; color: #8a8a86; }
